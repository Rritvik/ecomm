...
class OrderResource extends Resource
{
        ...
        return $form
            ->schema([
                ...
                ->schema([
                    ...
                    ->schema([
                        // Repeater::make('Name of the relationship')
                        Repeater::make('items')
                        ->relationship()
                        ->schema([
                            ...
                            ->columnSpan(4)
                            ->reactive()
                            // Set unit value of product on product selection
                            ->afterStateUpdated(
                                fn($state, Set $set) => $set('unit_amount', Product::find($state)?->price ?? 0)
                            )
                            // Set total amount of product on product selection
                            ->afterStateUpdated(
    ...
    public static function getNavigationBadge() : ?string {
        // Implement your custom logic here to return the badge count, if any.
